...
import { FaSyncAlt, FaFire, FaTag, FaPlus, FaTrash, FaPowerOff, FaEnvelope } from "react-icons/fa";
import { adminDiscountApi } from "../../Services/discountApi";
import { DiscountCode, DISCOUNT_ROLE_PERSIAN, TourSpecialDiscount } from "../../Types/discount";
import "../../Pages/ceo/discount/Discounts.css";
...
const AdminDiscounts: React.FC = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    setError("");
    try {
      const [spRes, cdRes] = await Promise.all([
        adminDiscountApi.getSpecials(),
        adminDiscountApi.getCodes(),
      ]);
  ...
  const handleDeleteSpecial = async (s: TourSpecialDiscount) => {
    if (!window.confirm(`تخفیف ویژه «${s.tourName}» (آژانس ${s.createdByUsername}) حذف شود؟`)) return;
    setActionLoading(true);
    try {
      const res = await adminDiscountApi.deleteSpecial(s.id);
      if (res.success) {
  ...
  const handleToggleSpecial = async (s: TourSpecialDiscount) => {
    setActionLoading(true);
    try {
      const res = await adminDiscountApi.toggleSpecial(s.id, !s.isActive);
      if (res.success) {
        showNotice("✓ وضعیت تغییر کرد");
        fetchAll();
      }
    } catch (err: any) {
  ...
  const handleCreateCode = async () => {
    ...
    setError("");
    try {
      const res = await adminDiscountApi.createCode({
        code: codeVal.trim(),
      ...
      if (res.success) {
        showNotice("✓ کد تخفیف ساخته شد" + (res.data.forUserUsername ? " و به اینباکس کاربر ارسال شد" : ""));
  ...
  const handleToggleCode = async (c: DiscountCode) => {
    setActionLoading(true);
    try {
      const res = await adminDiscountApi.toggleCode(c.id, !c.isActive);
      if (res.success) {
  ...
  const handleDeleteCode = async (c: DiscountCode) => {
    ...
    setActionLoading(true);
    try {
      const res = await adminDiscountApi.deleteCode(c.id);
      if (res.success) {
  ...
  return (
    <div className="dc-container">
      ...
      <div className="dc-header">
        <div>
          <h1>🛡️ مانیتورینگ تخفیف‌ها</h1>
          <p>تورهای ویژه و کدهای تخفیف همه آژانس‌ها + ساخت کد و ارسال به اینباکس کاربر</p>
        </div>
      ...
      <div className="dc-tabs">
        ...
        >
          <FaFire /> تورهای ویژه {specials.length > 0 && <span className="dc-tab-count">{specials.length}</span>}
        </button>
        <button
          className={`dc-tab ${activeTab === "codes" ? "active" : ""}`}
          onClick={() => setActiveTab("codes")}
        >
          <FaTag /> کدهای تخفیف {codes.length > 0 && <span className="dc-tab-count">{codes.length}</span>}
        </button>
      </div>
      ...
      {activeTab === "special" && (
        specials.length === 0 ? (
          <div className="dc-table-card">
            <div className="dc-empty">هیچ تخفیف ویژه‌ای در سیستم فعال نیست</div>
          </div>
        ) : (
            ...
            {specials.map((s) => (
              <div className="dc-card" key={s.id}>
                <div className="dc-card-head">
                  <div>
                    <h4>{s.tourName}</h4>
                    <small>{s.tourCode} — 🏢 {s.createdByUsername}</small>
                  </div>
      ...
      {activeTab === "codes" && (
        <>
                ...
                <div className="dc-form-body">
                  ...
                  <div className="dc-form-group">
                    ...
                    <input
                      ...
                      placeholder="مثلاً هدیه ویژه کاربر"
                    />
                  </div>
                  <div className="dc-form-group">
                  ...
                  <div className="dc-form-hint">
                    <FaEnvelope /> با پر کردن «گیرنده»، کد به اینباکس همان کاربر ارسال می‌شود و فقط او می‌تواند استفاده کند.
                  </div>
                </div>
                <div className="dc-form-actions">
                  <button className="dc-btn-submit" disabled={actionLoading} onClick={handleCreateCode}>
                    {actionLoading ? "⏳..." : "ساخت و ارسال به اینباکس"}
                  </button>
                </div>
              </>
            ) : (
          ...
          <div className="dc-table-card">
            {codes.length === 0 ? (
              <div className="dc-empty">هنوز کد تخفیفی در سیستم ساخته نشده</div>
            ) : (
                      ...
                      <tr key={c.id}>
                        ...
                        <td>
                          ...
                          <small style={{ color: "#94a3b8" }}>
                            ({DISCOUNT_ROLE_PERSIAN[c.createdByRole] || c.createdByRole})
                          </small>
                        </td>
